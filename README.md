...
从 SillyTavern 同步资源到插件私有存储时的去重策略：

| 资源类型 | 去重方式 | 同名更新行为 |
|----------|----------|-------------|
| **角色卡** | 时间戳 + 文件大小 | 覆盖（如果源文件较新） |
| **世界书** | 时间戳 + 文件大小 | 覆盖（如果源文件较新） |
| **预设** | 时间戳 + 文件大小 | 覆盖（如果源文件较新） |
| **快速回复** | 时间戳 + 文件大小 | 覆盖（如果源文件较新） |
| **全局正则** | 内容签名 | 创建新版本文件（如 `global__脚本名__1.json`） |

### 同步流程

```
SillyTavern 原始数据              插件私有存储
(data/default-user/)    ──同步──>  (plugins/ST-Manager-serverplugin/data/library/)
├── characters/*          →        ├── characters/*
├── worlds/*              →        ├── lorebooks/*
├── OpenAI Settings/*     →        ├── presets/*
├── regex/*               →        └── extensions/regex/*
└── settings.json 全局正则 →            └── global__*.json
```

设计意图：
- **普通资源**：基于文件名判断，源文件更新时覆盖目标
- **全局正则**：因存储在 settings.json 中，导出为独立 JSON 文件，支持多版本保留
