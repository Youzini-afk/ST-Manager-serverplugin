...
/**
 * 启动独立的 Web UI 服务器
 * 一比一复刻 Python 后端的完整 Web UI
 * 使用 Nunjucks 渲染 Jinja2 兼容的模板
 */
/**
 * 启动独立的 Web UI 服务器
 * 一比一复刻 Python 后端的完整 Web UI
 * 使用 Nunjucks 渲染模板，API 兼容层处理所有请求
 */
function startWebUIServer() {
    try {
        ...
        if (!fs.existsSync(templatesDir)) {
            console.error('[ST Manager] Web UI 模板目录不存在:', templatesDir);
            console.log('[ST Manager] 请确保 server/webui/templates 目录存在');
            return;
        }
        ...
        webApp.get('/', (req, res) => {
            try {
                ...
                console.error('[ST Manager] 模板渲染错误:', e.message);
                res.status(500).send('模板渲染错误: ' + e.message);
            }
        });
        ...
        webApp.listen(WEB_PORT, () => {
            ...
            console.log(`  📡 SillyTavern API: http://localhost:${stPort}/api/plugins/st-manager`);
            console.log('═══════════════════════════════════════════════');
            console.log('');
        });
    } catch (error) {
